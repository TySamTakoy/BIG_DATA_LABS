...
def mapper_1(transaction):
    """
    Преобразует транзакцию в пару (ключ, сумма).
    Ключ — fraud_amount, если транзакция мошенническая, иначе normal_amount.

    :param transaction: Словарь с данными одной транзакции
    :yield: Кортеж (ключ, сумма)
    """
...
# Функция shuffler
def shuffler(mapped_values):
    """
    Группирует значения по ключам.

    :param mapped_values: Список кортежей (ключ, значение)
    :return: defaultdict(list), сгруппированные по ключу данные
    """
...
def reducer_1(shuffled_data):
    """
    Вычисляет среднее значение для каждой категории транзакций.

    :param shuffled_data: defaultdict с ключами 'fraud_amount' и 'normal_amount'
    :return: Словарь со средними значениями
    """
    results = {}
...
def mapper_2(transaction):
    """
    Преобразует мошенническую транзакцию в пару (тип, 1) для подсчета.

    :param transaction: Словарь с транзакцией
    :yield: Кортеж ('fraud_type_ТипОперации', 1)
    """
    #проверка транзакции на мошенничество
    is_fraud = transaction['isFraud'] == '1'
    if is_fraud:
...
# Функция shuffler из задачи сравнения среднего размера мошеннических и обычных транзакций
...
def reducer_2(shuffled_data):
    """
    Подсчитывает количество мошеннических транзакций по каждому типу и сортирует по убыванию.

    :param shuffled_data: defaultdict, где ключ — тип, значение — список единиц
    :return: Словарь с отсортированным списком типов мошенничества
    """
    results = {}
    type_counts = {}
...
def main(file_path):
    ...
    shuffled_data_1 = shuffler(mapped_values_1)
    shuffled_data_2 = shuffler(mapped_values_2)
